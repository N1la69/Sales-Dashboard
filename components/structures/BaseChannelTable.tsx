...
export default function BaseChannelTable({
  ...
  const normalized: BreakdownItem[] = data.map((raw) => {
    const key = raw.key ?? raw.base_channel ?? raw.name ?? "Unknown";
    const name = raw.name ?? raw.base_channel ?? raw.key ?? "Unknown";
    ...
    <Card className="shadow-md bg-slate-100/40 dark:bg-slate-800/30 border-transparent">
      ...
      <CardContent className="overflow-x-auto">
        <table className="min-w-full text-sm text-left border-separate border-spacing-y-2">
          ...
          <tbody>{renderRows(sortedTop, 0, "base_channel")}</tbody>
        </table>
      </CardContent>
    </Card>
  );
